...
class CompuzoneParser:
    ...
    def _check_brand_match(self, product_name: str, maker_codes: List[str]) -> bool:
        """동적 브랜드 매칭 검사 - 유연하고 정확한 방식"""
        if not maker_codes:
            return True
            
        # 제품명에서 [브랜드] 추출
        bracket_brand_match = re.search(r'\[([^\]]+)\]', product_name)
        if not bracket_brand_match:
            return False  # 브랜드 정보가 없으면 필터링
            
        bracket_brand = bracket_brand_match.group(1).strip()
        
        # 1. 직접 매칭 (가장 정확)
        for code in maker_codes:
            if code.upper() == bracket_brand.upper():
                return True
                
        # 2. 브랜드 ID 매핑 (알려진 브랜드만)
        known_brands = self._get_brand_mapping()
        for code in maker_codes:
            if code.isdigit() and code in known_brands:
                expected_brand = known_brands[code]
                if self._brands_match(bracket_brand, expected_brand):
                    return True
                    
        # 3. 유연 매칭 (유사도 체크)
        for code in maker_codes:
            if self._flexible_brand_match(bracket_brand, code):
                return True
                
        return False
        
    def _get_brand_mapping(self) -> dict:
        """알려진 브랜드 ID 매핑 (정확한 매칭만 포함)"""
        return {
            # 주요 저장장치 브랜드
            '2': '삼성전자', '6202': '삼성전자',
